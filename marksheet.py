...
# label to enter name
name=tk.Label(master, text="Name").grid(row=0, column=0)

# label for registration number
regno=tk.Label(master, text="Reg.No").grid(row=0, column=3)

# label for roll Number
rollno=tk.Label(master, text="Roll.No").grid(row=1, column=0)
...
tk.Label(master, text="Operating Systems").grid(row=5, column=1)
tk.Label(master, text="Data Structures").grid(row=6, column=1)


# label for grades
tk.Label(master, text="Grade Obtained").grid(row=2, column=2)
...
tk.Label(master, text="Max. Credits").grid(row=2, column=3)
...
totcred=tk.Label(master, text="Total credit").grid(row=7, column=3)
totsgpa=tk.Label(master, text="SGPA").grid(row=8, column=3)

mywidgets=[(name,e1),(regno,e2),(rollno,e3),(totcred,tot),(totsgpa,sgpa)]
def get_entry():
        with open('output.txt','w') as out:
                for label, entry in mywidgets:
                        out.write(label['text']+entry.get()+'\n')
                        
button2=tk.Button(master, text="Add to Register", bg="yellow", command=get_entry)
